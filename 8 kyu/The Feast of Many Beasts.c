// -----------------------------------------------------------
// All of the animals are having a feast! Each animal is bringing one dish. There is just one rule: the dish must start and end with 
// the same letters as the animal's name. For example, the great blue heron is bringing garlic naan and the chickadee is 
...
bool feast(const char* beast, const char* dish){
  if (!beast || !dish || !*beast || !*dish) return false;
  size_t len_beast = strlen(beast);
  size_t len_dish = strlen(dish);
  return beast[0] == dish[0] && beast[len_beast-1] == dish[len_dish-1];
}
